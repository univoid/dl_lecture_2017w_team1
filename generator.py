...
class Generator(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
                 sequence_length, cond_length, start_token,
                 learning_rate=0.01, reward_gamma=0.95, is_cond=0):
        ...
        if self.is_cond:
            self.cond = tf.placeholder(tf.int32, shape=[self.batch_size, self.cond_length])
            self.cond_vector = tf.nn.embedding_lookup(self.g_embeddings, self.cond) # batch_size x cond_length x emb_dim
            sums = tf.reduce_sum(self.cond_vector, axis=1, keep_dims=False)
    ...
    def generate(self, sess, cond=None):
        if self.is_cond:
            outputs = sess.run(self.gen_x, feed_dict={self.cond: cond})
        else:
            outputs = sess.run(self.gen_x)
        return outputs

    def pretrain_step(self, sess, x, cond=None):
        if self.is_cond:
            feed = {self.x: x, self.cond: cond}
        else:
            feed = {self.x: x}
